...
interface SaleDao {
    ...
    @Query("SELECT * FROM sales WHERE fk_tenant = :tenantId AND sync_status = 'conflict'")
    suspend fun getConflicted(tenantId: String): List<SaleEntity>

    @Query("UPDATE sales SET sync_status = 'synced' WHERE id IN (:ids)")
    suspend fun markSynced(ids: List<String>)

    @Query("UPDATE sales SET sync_status = 'conflict' WHERE id IN (:ids)")
