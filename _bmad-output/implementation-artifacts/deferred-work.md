...
## Deferred from: code review de 4-1-motor-de-sincronizacion-push-en-background — backend (2026-08-09)

- **Sin límite de tamaño de lote en el payload de `POST /api/v1/sync/push/{entity}`** — un array sin cota de elementos puede disparar un `SaveChangesAsync` de tamaño arbitrario en una sola transacción, sin paginación ni guard de tamaño de request. Hardening pendiente para una historia futura. [SyncService.cs]
- **DDL crudo interpola `{schema}` directamente en 8 tablas más** — extiende el patrón ya diferido desde Historia 1.4 (antes solo `settings`), ahora con más peso. Consolidar estrategia de migraciones sigue pendiente. [ApplicationBuilderExtensions.cs]
- **Doble fuente de verdad del esquema de las 9 tablas sincronizables** — el fluent mapping de `TenantDbContext.OnModelCreating` y el DDL crudo de `ApplicationBuilderExtensions` deben mantenerse manualmente sincronizados; nada lo garantiza. Mismo ítem diferido desde Historia 1.4, ahora cubriendo 9 tablas en vez de 2. [TenantDbContext.cs, ApplicationBuilderExtensions.cs]
- **DDL no declara `FOREIGN KEY` en ninguna de las 8 tablas nuevas** (solo `PRIMARY KEY`) — filas huérfanas son posibles si el orden de sync se rompiera o el cliente enviara datos inconsistentes; baja probabilidad hoy porque `SyncManager.pushPending` respeta el orden de dependencia (clientes→productos→variantes→ventas→...), pero sin garantía a nivel BD. [ApplicationBuilderExtensions.cs]
- **Race de "mismo Id concurrente" en `SyncService`** — `FindAsync` seguido de `Add`/update no es una operación atómica; dos requests HTTP concurrentes pusheando el mismo `Id` nuevo podrían chocar en `SaveChangesAsync`. Misma clase de riesgo ya aceptada explícitamente para v1 en las 3 condiciones de carrera diferidas por la retro de Epic 3 (single-device-per-tenant); aquí aplica al servidor recibiendo requests concurrentes en vez del dispositivo. Revisar junto con esas 3 cuando Epic 4 introduzca concurrencia real. [SyncService.cs]
- **`EnsureTenantSchemaAsync` aborta el aprovisionamiento de TODOS los tenants si uno solo falla en el arranque** — no es específico de esta historia (el bucle ya existía), pero ahora protege 9 tablas en vez de 2, con más superficie de fallo. [ApplicationBuilderExtensions.cs]

## Deferred from: code review de 4-1-motor-de-sincronizacion-push-en-background — Android (2026-08-09)

- **Sin límite de tamaño de lote en el push del cliente Android** + `writeTimeout` por defecto de OkHttp (10s, sin override explícito) puede ser corto para lotes grandes tras un periodo offline extendido — misma clase de hardening ya diferida del lado backend (`SyncService`). [SyncManager.kt, NetworkModule.kt]
- **Excepciones silenciadas sin logging/telemetría en `SyncManager`/`PushWorker`** — todo `runCatching { ... }.isSuccess` descarta el throwable capturado sin `Log.e` ni ningún hook de crash reporting; no hay forma de diagnosticar por qué falló una corrida de sync desde los logs del dispositivo. Requiere decidir un framework de logging para el proyecto (no existe ninguno establecido todavía). [SyncManager.kt, PushWorker.kt]
- **`SettingsRepository.downloadAndCacheSettings` resetea `created_at` a "ahora" en cada re-descarga**, descartando el `createdAt` real de una descarga previa — hoy inconsecuente porque no existe ningún flujo que edite un Setting localmente (la única escritura es esta descarga, que siempre marca `synced` de inmediato), pero la semántica queda incorrecta en cuanto se agregue edición local de Settings. [SettingsRepository.kt]

## Deferred from: code review de 3-7-credito-a-favor-y-cancelacion-de-venta-con-cobros (2026-08-04)

- **`OrderDetailViewModel.onCancelOrderConfirm` decide con `paymentHistory` cacheado en `uiState`, no con una lectura fresca del cliente** — un cobro registrado por otra vía entre la carga de S-09 y el tap en "Cancelar Orden" podría cancelar la venta directo (AC-1) sin ofrecer el segundo dialog Opción A/B (AC-2), dejando ese cobro sin resolver explícitamente. Misma familia que la condición de carrera de `registerPayment` de abajo — inalcanzable hoy (un solo dispositivo/sesión, sin escritura concurrente real). Revisar cuando Epic 4 introduzca sincronización multi-dispositivo. [OrderDetailViewModel.kt]

## Deferred from: code review de 3-6-registro-de-cobros-sobre-ventas-y-parcialidades (2026-08-02)

- **Condición de carrera en `SaleRepository.registerPayment`** — dos llamadas concurrentes podrían ambas leer el mismo estado "pendiente" antes de que cualquiera escriba, duplicando el cobro (dos `Payment` sumando el doble del total, la venta quedando marcada `paid` una sola vez). Baja probabilidad hoy: la app es de un solo dispositivo/sesión local, sin motor de sincronización ni escritura concurrente real todavía. Revisar cuando Epic 4 (sincronización offline) introduzca escritura multi-dispositivo genuina — misma revisión pendiente ahora también cubre el consumo FIFO de crédito en `createSale` (Historia 3.7), con la misma condición de carrera read-then-write. [SaleRepository.kt, RegisterPaymentUseCase.kt]
